...
class NNPrediction:
    def __init__(self, model='res_fcn'):
        self.cv_bridge = CvBridge()
        r = rospkg.RosPack()
        path = r.get_path('nn_predict')
        self.labels = ['background', 'barcode']
        model_name, self.network = self.build_nn(model)
        if self.network is None:
            rospy.logerr("Network not found!")
            return
